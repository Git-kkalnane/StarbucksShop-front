...
export const OrderItemList: React.FC<{ items: OrderItem[] }> = ({ items }) => {
    const calculateItemTotal = (item: OrderItem) => {
        const optionsTotal = item.options?.reduce((sum, opt) => sum + opt.additionalPrice * opt.quantity, 0) || 0;
        return (item.itemPrice + optionsTotal) * item.orderItemQuantity;
    };

    console.log('OrderItemList items: ', items);
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: '10px' }}>
                ...
                <tbody>
                            ...
                            <tr>
                                ...
                                >
                                    ...
                                    {item.options && item.options.length > 0 && (
                                        <div style={{ color: '#666' }}>
                                            {item.options
                                                .reduce((sum, opt) => sum + opt.additionalPrice * opt.quantity, 0)
                                                .toLocaleString()}
                                            원
                                        </div>
                                    )}
                    ...
                    <tr>
                        ...
                        >
                            {items.reduce((sum, item) => sum + calculateItemTotal(item), 0).toLocaleString()}원
                        </td>
                    </tr>
                </tbody>
            </table>
